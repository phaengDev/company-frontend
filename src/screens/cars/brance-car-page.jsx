...
function BrancecarPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(inputs)
    try {
      axios.post(api + 'brands/create', inputs)
  ...
  const [itemBrands, setItimeBrands] = useState([]);
  const fetchBrands = async () => {
    try {
      const response = await fetch(api + 'brands');
      const jsonData = await response.json();
      setItimeBrands(jsonData);
      setData(jsonData);
    } catch (error) {
  ...
  //====================
  const Filter = (event) => {
    setItimeBrands(data.filter(n => n.brands_name.toLowerCase().includes(event)))
  }
  ...
  const pages = [];
  for (let i = 1; i <= Math.ceil(itemBrands.length / itemsPerPage); i++) {
    pages.push(i);
  }

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = itemBrands.slice(indexOfFirstItem, indexOfLastItem);

  const [i, setI] = useState(1);
  const qtyItem = itemBrands.length;
  ...
  return (
    <>
      <div id="content" class="app-content">
        ...
        <h3 class="page-header fs-20px">ຂໍ້ມູນຍີ່ຫໍ້ລົດ</h3>
        ...
        <Modal open={open} onClose={handleClose}>
          <Modal.Header>
            <Modal.Title className='pt-1'>ເພີ່ມຂໍ້ມູນຍີ່ຫໍ້ລົດ</Modal.Title>
          </Modal.Header>
