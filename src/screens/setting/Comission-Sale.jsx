...
export default function ComissionSale() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post(api + 'comisget/create', inputs)
            .then(response => {
    ...
    const fetchCommission = async () => {
        try {
            const response = await axios.post(api + 'comisget/fetch', values);
    ...
    const headleEdit = (item) => {
        setOpened(true);
        setVal({
            comis_oac_id: item.comis_oac_id,
    ...
    const handleSubmitEdit = (e) => {
        e.preventDefault();
        axios.put(api + 'comisget/edit', val)
            .then(response => {
                if (response.status === 200) {
                    setOpened(false);
                    fetchCommission();
    ...
    const headleDelete = (id) => {
        axios.delete(api + `comisget/${id}`).then(function (response) {
            if (response.status === 200) {
                fetchCommission()
                toaster.push(
                            ...
                            <tbody>
                                {
                                    currentItems.length > 0 ? (
                                        ...
                                        <tr>
                                            <td colSpan={5} className='text-center text-red'> ບໍ່ພົບຂໍ້ມູນທີ່ມີການທີ່ມີການບັນທຶກ</td>
                                        </tr>
                                    )}
                            </tbody>
                        </table>
